...
void PrintBmpData (long temperature, long pressure);
short bmp085ReadShort (unsigned char address);
void I2CGetBytes (unsigned char address, unsigned char bytes, char *buff, char ack);
/** VECTOR REMAPPING ***********************************************/
#if defined(__18CXX)
...
{   
    ...
    while(1)
    {
        ...
        ProcessIO();        
		bmp085Convert (&BmpUt, &BmpUp);
		//if (USBUSARTIsTxTrfReady())
			PrintBmpData (BmpUt, BmpUp);
    }//end while
}//end main
/******************************************************************************
 * Function:        void I2CGetBytes (unsigned char address, 
 *					unsigned char bytes, unsigned char *buff)
 *
 * PreCondition:    None
 *
 * Input:           Slave device address, number of bytes to be read,
 *					destination buffer
 *
 * Output:          None
 *
 * Side Effects:    None
 *
 * Overview:        This routine gets the number of bytes and store into the
 *					given buffer.
 *
 * Note:            
 *
...
/******************************************************************************
 * Function:        short bmp085ReadShort (unsigned char address)
 *
 * PreCondition:    None
 *
...
short bmp085ReadShort (unsigned char address)
{
	...
	SendStop();
	//SendStart();
	I2CGetBytes (((BMP085 << 1) | 0x01), 2, &_buff, 0);
	//SendStop();
	data = _buff[0];
	data = data << 8;
	data |= _buff[1];
...
void PrintBmpData (long temperature, long pressure)
{
	...
	NextUSBOut++;
	strcpy (USB_Out_Buffer+NextUSBOut, _buff);
	NextUSBOut = strlen (USB_Out_Buffer);
	USB_Out_Buffer[NextUSBOut] = 'T';
	NextUSBOut ++;
	ltoa (temperature, _buff);
	strcpy (USB_Out_Buffer+NextUSBOut, _buff);	
	NextUSBOut = strlen (USB_Out_Buffer);
...
{
	long up = 0;
	unsigned char ossValue;
	SendStart();
	SendByte (BMP085 << 1);
	SendByte (0xF4);
	ossValue = OSS << 6;
	ossValue = 0x34+ossValue;
	SendByte (ossValue);
	SendStop();
	while (!(PORTCbits.RC5));
	SendStart();
	...
	SendByte (0xF6);
	SendStop();
	I2CGetBytes (((BMP085 << 1) | 0x01), 3, &_buff, 0);
	up = (unsigned long)_buff[0];
	up = up << 8;
	up |= (unsigned long)_buff[1];
	up = up << 8;
	up |= (unsigned long)_buff[2];
	//up &= 0xFFFF00;
	ossValue = 8-OSS;
	up = up >> ossValue;
	//up = up >> 8;
...
void bmp085Convert(long* temperature, long* pressure)
{
	...
	//ut = 27898;
	//up = 23843;
	x1 = (((long)ut - (long)ac6) * (long)ac5) / 0x8000;
	x2 = ((long)mc * 0x800) / (x1 + md);
	b5 = x1 + x2;
	*temperature = (b5 + 8) / 0x10;
	...
	//return;
	b6 = b5 - 4000;
	x1 = ((long)b2 * (b6 * b6 / 0x1000)) / 0x800;
	x2 = (long)ac2 * b6 / 0x800;
	x3 = x1 + x2;
	b3 = (((long) ac1) * 4 + x3);
	b3 = b3 << OSS;
	b3 = (b3+2)/0x04;
	//b3 = ((((long) ac1) * 4 + x3) + 2)/ 0x04;
	x1 = (long)ac3 * b6 / 0x2000;
	x2 = (b1 * (b6 * b6 / 0x1000)) / 0x10000;
	x3 = ((x1 + x2) + 2) / 4;
	b4 = ((unsigned long)ac4 * (unsigned long) (x3 + 0x8000)) / 0x8000;
...
/******************************************************************************
 * Function:        void InitializeI2C(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        This routine sets i2c as master at 10kHz
 *
 * Note:            
 *
 *****************************************************************************/
 ...
 * Side Effects:    None
 *
 * Overview:        Waits for SSPIF to clear
 *
 * Note:            
 *
 *****************************************************************************/
...
/******************************************************************************
 * Function:        void SendRestart(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Overview:        Sends i2c restart bit
 *
 * Note:            
 *
 *****************************************************************************/
 ...
 * Side Effects:    None
 *
 * Overview:        Sends byte to slave and wait for ack
 *
 * Note:            
 *
 *****************************************************************************/
 ...
 * Input:           None
 *
 * Output:          Byte from slave device
 *
 * Side Effects:    None
 *
 * Overview:        Gets byte from slave device
 *
 * Note:            
 *
 *****************************************************************************/
 ...
 * Side Effects:    None
 *
 * Overview:        Sends nack to slave device
 *
 * Note:            
 *
 *****************************************************************************/
 ...
 * Side Effects:    None
 *
 * Overview:        Sends ack to slave device
 *
 * Note:            
 *
 *****************************************************************************/
